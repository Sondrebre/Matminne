...
public class StripeController {
    ...
    public String aktiverKampanje(@RequestParam String kode,
                                   @AuthenticationPrincipal OAuth2User principal) {
        if (principal == null) return "redirect:/";
        if ("test123".equalsIgnoreCase(kode.trim())) {
        ...
        return "redirect:/abonnement?feil=ugyldig&kode=" + kode;
    }
    ...
    public ResponseEntity<String> handleWebhook(
        ...
        if (webhookSecret.isBlank()) {
            log.warn("Webhook-secret mangler, hopper over signatursjekk");
        ...
        switch (event.getType()) {
            ...
            case "customer.subscription.paused": {
                // Deaktiver abonnement når det avsluttes/pauses i Stripe
                event.getDataObjectDeserializer().getObject().ifPresent(obj -> {
                    com.stripe.model.Subscription sub = (com.stripe.model.Subscription) obj;
                    String customerId = sub.getCustomer();
                    brukerRepository.findAll().stream()
                            .filter(b -> customerId.equals(b.getStripeCustomerId()))
                            .findFirst()
                            .ifPresent(b -> {
            ...
            case "invoice.payment_succeeded": {
                // Reaktiver ved fornyet betaling
                event.getDataObjectDeserializer().getObject().ifPresent(obj -> {
